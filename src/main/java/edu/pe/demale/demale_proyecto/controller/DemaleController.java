...
public class DemaleController { 
    ...
    /**
     * Endpoint GET para obtener el historial de productos con capacidades de búsqueda y filtrado.
     ...
     * - startDate: para filtrar por fecha de ingreso a partir de. Formato ISO 8601 (ej. 2025-05-20T10:00:00).
     * - endDate: para filtrar por fecha de ingreso hasta. Formato ISO 8601 (ej. 2025-05-25T23:59:59).
     ...
     * @return ResponseEntity con una lista de DemaleModel que coinciden con los criterios,
    ...
    public ResponseEntity<List<DemaleModel>> getProductHistory(
            @RequestParam(required = false) String search, // Parámetro de URL opcional para búsqueda (ej. ?search=Zapatos)
            @RequestParam(required = false) String status, // Parámetro de URL opcional para filtro por estado (ej. ?status=En Proceso)
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime startDate, // Parámetro de URL opcional para fecha de inicio
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime endDate) { // Parámetro de URL opcional para fecha de fin

        // Llama al método del servicio para obtener los movimientos de productos con los filtros
